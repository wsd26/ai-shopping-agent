...
function classifyIntent(userText: string): string {
  return shoppingAgent.process({ userText, ...emptyInput }).intent
}
...
describe('Intent Recognition Evaluation', () => {
  ...
  describe('greeting', () => {
    ...
    it.each(exactGreetings)('"%s" → greeting', (text) => {
      expect(classifyIntent(text)).toBe('greeting')
    })

    const ambiguousGreetings = ['嗨', '在不在', '来了', '大家好']
    it.each(ambiguousGreetings)('"%s" → greeting or general', (text) => {
      const intent = classifyIntent(text)
      expect(['greeting', 'general', 'clarify']).toContain(intent)
    })
  })
  ...
  describe('specific_product', () => {
    ...
    it.each(cases)('"%s" → specific_product or related', (text) => {
      const intent = classifyIntent(text)
  ...
  describe('product_search', () => {
    ...
    it.each(clearSearch)('"%s" → product_search', (text) => {
      expect(classifyIntent(text)).toBe('product_search')
    })

    // These may find products via category fallback (product_search → specific_product)
    const ambiguousSearch = [
      '有没有男装', '有女装吗', '有零食吗', '有数码产品吗', '有没有包包',
      '推荐鞋子', '有没有男士衣服', '有裙子吗', '有外套吗', '有防晒吗',
      '想要面膜', '推荐护肤品', '有T恤吗', '想买精华', '有耳机吗', '有配饰吗',
    ]
    it.each(ambiguousSearch)('"%s" → product_search or specific_product', (text) => {
      const intent = classifyIntent(text)
  ...
  describe('current_product', () => {
    ...
    it.each(cases)('"%s" → current_product', (text) => {
      const intent = classifyIntent(text)
  ...
  describe('command', () => {
    ...
    it.each(clearCommands)('"%s" → command', (text) => {
      expect(classifyIntent(text)).toBe('command')
    })

    const commandWithCurrent = ['买这个', '就要这个', '就要了', '买了']
    it.each(commandWithCurrent)('"%s" → command', (text) => {
      expect(classifyIntent(text)).toBe('command')
    })

    const ambiguousCommands = ['加购', '帮我看一下', '加到购物车']
    it.each(ambiguousCommands)('"%s" → command or general', (text) => {
      const intent = classifyIntent(text)
  ...
  describe('general', () => {
    ...
    it.each(cases)('"%s" → non-null intent', (text) => {
      const intent = classifyIntent(text)
  ...
  // ─── Summary statistics ───
  it('intent analysis coverage report', () => {
    ...
    for (const [text, expected] of allCases) {
      const predicted = classifyIntent(text)
