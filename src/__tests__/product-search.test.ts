...
describe('Product Search Evaluation', () => {
  ...
  describe('honest not found — returns empty for missing items', () => {
    it('"手表" not in catalog → empty', () => {
      const results = advisorAgent.searchProducts(['手表'], null)
      expect(results.length).toBe(0)
    })

    it('"家具" not in catalog → empty', () => {
      const results = advisorAgent.searchProducts(['家具'], null)
      expect(results.length).toBe(0)
    })

    it('"防晒霜" not in catalog → empty (only 精华液 exists)', () => {
      const results = advisorAgent.searchProducts(['防晒'], 'skincare')
      // No sunscreen product; "防晒" partial chars don't match existing skincare
      expect(results.length).toBe(0)
    })

    it('"汽车" not in catalog → empty', () => {
      const results = advisorAgent.searchProducts(['汽车'], null)
      expect(results.length).toBe(0)
    })
  })
  ...
  describe('category filtering', () => {
    ...
    it('null category searches all categories', () => {
      const results = advisorAgent.searchProducts(['面膜'], null)
      // May find "面" partial match in non-skincare too
  ...
  it('product search evaluation report', () => {
    ...
    // Test queries designed for the actual product catalog
    ...
    for (const q of testQueries) {
      const results = advisorAgent.searchProducts(q.keywords, q.category)
    ...
    expect(passed).toBeGreaterThanOrEqual(9) // At least 9/11 pass
  })
})
